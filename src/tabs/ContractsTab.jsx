...
function ContractDetail({ contract, onClose, onOpenCompany }) {
  ...
  return (
    <Modal title={`Contrat — ${co?.company || "?"}`} onClose={onClose}>
      ...
      <div style={{ ...S.cT, marginTop: 14 }}>💳 {fmt(paid)} / {fmt(totTTC)}</div>
      {totTTC > 0 && <div style={S.barBox}><div style={S.bar((paid / totTTC) * 100, Cl.ok)} /></div>}
      {(contract.payments || []).map(p => (
        <div key={p.id} style={{ display: "flex", alignItems: "center", gap: 6, padding: "4px 0", borderBottom: `1px solid ${Cl.brd}`, fontSize: 12, opacity: p.status === "Payé" ? 0.6 : 1 }}>
          <span style={{ flex: 1 }}>{p.label} — <strong>{fmt(p.amount)}</strong> · {p.dueDate || "—"}</span>
          <select style={{ ...S.sel, width: "auto", fontSize: 11, padding: "2px 6px", borderRadius: 12, fontWeight: 600, background: p.status === "Payé" ? Cl.okL : p.status === "En retard" ? Cl.errL : Cl.warnL }} value={p.status} onChange={e => upd({ payments: contract.payments.map(x => x.id === p.id ? { ...x, status: e.target.value } : x) })}><option>En attente</option><option>Payé</option><option>En retard</option></select>
        </div>
      ))}
